    ...
    <div ref={containerRef} className="relative flex items-center gap-2">
      <div className={!sandboxInfo ? "opacity-55" : ""}>
        <div className="flex h-8 items-center rounded-lg border border-border bg-muted p-0.5 overflow-hidden">
          ...
          >
            ...
            <span className="hidden @[220px]:inline">Remote IDE: {getIdeLabel()}</span>
          ...
          >
            <ChevronRight
              className={
                "size-3.5 transition-transform " + (showDropdown ? "rotate-180" : "")
              }
            />
          </button>
        </div>
      ...
      {showDropdown && (
        <div className="absolute right-0 top-10 z-50 w-56 rounded-lg border bg-popover py-1.5 shadow-xl">
          ...
          {[SupportedIDEs.vscode, SupportedIDEs.cursor, SupportedIDEs.jetbrains].map(
            (ide) => (
              <button
                key={ide}
                type="button"
          ...
          <div className="px-2 pb-1">
            ...
            >
              <span className="max-w-[140px] truncate">{displaySshCommand}</span>
              {copied ? (
