...
class JavaValidationInput(BaseModel):
    ...
    validate_ast: bool = Field(
        default=True, description="Whether to use Serena MCP for deeper AST validation."
    )


@tool("validate_java_code", args_schema=JavaValidationInput)
async def validate_java_code(
    source_code: str, framework: str = "none", validate_ast: bool = True
) -> str:
    ...
    if validate_ast and serena_enabled:
        ...
        return (
            f"[Serena AST Validation Passed] Framework: {framework}. Symbols checked."
        )
