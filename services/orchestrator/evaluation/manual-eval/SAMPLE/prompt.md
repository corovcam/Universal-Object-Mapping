# Manual translation prompt — efcore-mongodb-q1 (SAMPLE)

| | |
|---|---|
| **Pair** | .NET Entity Framework Core → Java Spring Data MongoDB |
| **Translation type** | both |
| **Fixture** | `efcore-mongodb-q1 (SAMPLE)` (WideWorldImporters) |
...
7. You finish by calling the single `save_translation` tool EXACTLY ONCE with every required field filled. There is no separate JSON output. The required fields are:
   - source_validation_body: the query-execution harness BODY for the SOURCE side (see below).
   - target_validation_body: the query-execution harness BODY for the TARGET side (see below).
...
8. The `*_validation_body` fields are the RUNNABLE harness body ONLY: start directly at the schema/entity declarations, then the query classes, then the entrypoint class with `main`/`Main` that validates each entity and runs each query. The fixed boilerplate — `import`/`using`/`package`/`namespace` lines, the JSON serializer, the runtime-support and DB template-factory classes — is INJECTED FOR YOU around your body. Do NOT write those lines and do NOT redeclare those classes.
   - source_validation_body MUST declare the source entrypoint class named exactly `EFCoreQueryEntrypoint`.
   - target_validation_body MUST declare the target entrypoint class named exactly `MongoQueryEntrypoint`.
...
must follow for this framework pair. Each is the region BELOW the `// --- Schema and Related Settings ---` seam only: entity
...
<body_structure side="source" framework=".NET Entity Framework Core" entrypoint_class="EFCoreQueryEntrypoint">
[Table("Customers", Schema = "Sales")]
public class Customer
{
    [Key]
...
<body_structure side="target" framework="Java Spring Data MongoDB" entrypoint_class="MongoQueryEntrypoint">
/**
...
System time: 2026-06-28T13:08:23.149780+00:00
...
(filled in live by schema_inspection — a per-entity SQL→Mongo column mapping table)
---
Source Code:
<source_schema_code>
