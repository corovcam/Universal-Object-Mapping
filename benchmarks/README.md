...
Then, during query benchmarking I removed the Query execution on the DBMS by mocking or intercepting the command, and then constructing the underlying DbDataReader directly. The idea is to measure each ORM's time and memory overhead without actually executing the possibly long-running query on the database server. I tried to mock the ORMs as low-level as possible to contain the most of the overhead (method calling, memory allocation, etc.) as possible.
